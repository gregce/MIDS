...
## create an easy to reference shorthand for analysis dataset, also ensure to exclude invalid recall observations
d<- pd.cleaned %>%
...
recall.treat <- lm(recall_Question1CorrectAnswer ~ exp1_Treatment, d)

recall.ordertreat <- lm(recall_Question1CorrectAnswer ~ exp1_TreatmentRecallRollupOrdered, d)

recall.tuftetreat <- lm(recall_Question1CorrectAnswer ~ exp1_TreatmentRecallRollupTufte, d)
...
summary(lm(recall_PercentageCorrect ~ exp1_Treatment + gen_Gender + gen_Age + gen_Education + iq_PercentageCorrect, d))

summary(lm(recall_PercentageCorrect ~ exp1_TreatmentRecallRollupOrdered + recall_PageFiveTimeMinutes, d))
summary(lm(recall_PercentageCorrect ~ exp1_TreatmentRecallRollupOrdered + gen_Gender + gen_Age + gen_Education + iq_PercentageCorrect, d))

summary(lm(recall_PercentageCorrect ~ exp1_TreatmentRecallRollupTufte + recall_PageFiveTimeMinutes, d))
summary(lm(recall_PercentageCorrect ~ exp1_TreatmentRecallRollupTufte + gen_Gender + gen_Age + gen_Education + iq_PercentageCorrect, d))
