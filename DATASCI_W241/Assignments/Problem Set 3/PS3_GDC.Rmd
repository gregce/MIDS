...
## define a function to compute 95% confidence intervals
conf.ints <- function(model){
  mod <- summary(model)
...
conf.ints(lm(avg_bins_treat ~ bin + base_avg_bins_treat,d4))

##Problem C & D & E code all below
...
```{r}
conf.ints(lm(avg_bins_treat ~ bin,d4))
```
...
```{r}
conf.ints(lm(avg_bins_treat ~ bin + base_avg_bins_treat + factor(as.character(street)),d4))
```
...
```{r}
conf.ints(lm(avg_bins_treat ~ bin + base_avg_bins_treat + factor(as.character(street)) + nocell,d4))
```
